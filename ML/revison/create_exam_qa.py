...
data = [
    # --- Revision Worksheet Part 2 ---
    {
        ...
        "solution_en": "P(Y=1) = π, P(Y=0) = 1-π.<br/>Combined: P(Y=y) = π<super>y</super> (1-π)<super>1-y</super>.",
        "solution_cn": "P(Y=1) = π, P(Y=0) = 1-π。<br/>合并后：P(Y=y) = π<super>y</super> (1-π)<super>1-y</super>。"
    },
    {
        ...
        "solution_en": "Likelihood L = ∏<sub>i=1</sub><super>N</super> p(x<sub>i</sub>; y<sub>i</sub>).<br/>Negative log-likelihood = -∑<sub>i=1</sub><super>N</super> log p(x<sub>i</sub>; y<sub>i</sub>).",
        "solution_cn": "似然 L = ∏<sub>i=1</sub><super>N</super> p(x<sub>i</sub>; y<sub>i</sub>)。<br/>负对数似然 = -∑<sub>i=1</sub><super>N</super> log p(x<sub>i</sub>; y<sub>i</sub>)。"
    },
    {
        "topic": "Logistic Regression Optimization",
        "question_en": "Derive the derivative of the log-probability for logistic regression using π(x<sub>i</sub>) = Sigmoid(w<super>T</super> x<sub>i</sub>).",
        "question_cn": "使用 π(x<sub>i</sub>) = Sigmoid(w<super>T</super> x<sub>i</sub>) 推导逻辑回归对数概率的导数。",
        "solution_en": "Log-prob: y<sub>i</sub> log π(x<sub>i</sub>) + (1-y<sub>i</sub>) log(1-π(x<sub>i</sub>)). Differentiating requires iterative methods as it's non-linear.",
        "solution_cn": "对数概率：y<sub>i</sub> log π(x<sub>i</sub>) + (1-y<sub>i</sub>) log(1-π(x<sub>i</sub>))。由于是非线性的，微分求解需要迭代方法。"
    },
    {
        ...
        "solution_en": "Reverse mode: v&#772;<sub>i</sub> = ∂y<sub>j</sub> / ∂v<sub>i</sub> (gradient of output wrt node).<br/>Forward mode: v&#775;<sub>i</sub> = ∂v<sub>i</sub> / ∂x<sub>j</sub> (gradient wrt input).",
        "solution_cn": "反向模式：v&#772;<sub>i</sub> = ∂y<sub>j</sub> / ∂v<sub>i</sub>（输出相对于节点的梯度）。<br/>前向模式：v&#775;<sub>i</sub> = ∂v<sub>i</sub> / ∂x<sub>j</sub>（相对于输入的梯度）。"
    },
    {
    ...
    },
    {
        ...
        "solution_en": "Input: 30 × 45 = 1350. Output: 5 (one-hot encoding).",
        "solution_cn": "输入：30 × 45 = 1350。输出：5（独热编码）。"
    },
    {
        ...
        "solution_en": "Formula: O = (N - F + 2P)/S + 1<br/>For S=1: O = (28 - 5 + 4)/1 + 1 = 28.<br/>For S=2: O = (28 - 5 + 4)/2 + 1 = 14.5 → 14.",
        "solution_cn": "公式：O = (N - F + 2P)/S + 1<br/>对于 S=1：O = (28 - 5 + 4)/1 + 1 = 28。<br/>对于 S=2：O = (28 - 5 + 4)/2 + 1 = 14.5 → 14。"
    },
    {
        ...
        "solution_en": "1. Conv (F=7, P=3, S=1), 8 filters.<br/>2. ReLU.<br/>3. Conv (F=5, P=2, S=3), 16 filters (reduces size).<br/>4. ReLU.<br/>5. FC layer (input 2400, out 600).<br/>6. ReLU.<br/>7. FC layer (out 5).",
        "solution_cn": "1. 卷积 (F=7, P=3, S=1), 8个滤波器。<br/>2. ReLU。<br/>3. 卷积 (F=5, P=2, S=3), 16个滤波器 (减小尺寸)。<br/>4. ReLU。<br/>5. 全连接层 (输入 2400, 输出 600)。<br/>6. ReLU。<br/>7. 全连接层 (输出 5)。"
    },
    {
        ...
        "solution_en": "Maximise u<sub>1</sub><super>T</super> C u<sub>1</sub> + λ<sub>1</sub> (1 - u<sub>1</sub><super>T</super> u<sub>1</sub>). Maximise variance subject to normalisation constraint. Equivalent to minimising reconstruction error.",
        "solution_cn": "最大化 u<sub>1</sub><super>T</super> C u<sub>1</sub> + λ<sub>1</sub> (1 - u<sub>1</sub><super>T</super> u<sub>1</sub>)。在归一化约束下最大化方差。等同于最小化重构误差。"
    },
    {
    ...
    },
    {
        ...
        "solution_en": "Ncut(A,B) = Cut(A,B)/Vol(A) + Cut(A,B)/Vol(B).<br/>Where Cut is sum of weights between A and B, Vol is sum of degrees in the set.",
        "solution_cn": "Ncut(A,B) = Cut(A,B)/Vol(A) + Cut(A,B)/Vol(B)。<br/>其中 Cut 是 A 和 B 之间的权重之和，Vol 是集合中度的总和。"
    },
    {
    ...
    },
    {
        ...
        "solution_en": "H(Rain|Cloud)=0.88, H(Rain|NoCloud)=0.29.<br/>Expected H = 0.8 × 0.88 + 0.2 × 0.29 = 0.76 bits.",
        "solution_cn": "H(Rain|Cloud)=0.88, H(Rain|NoCloud)=0.29.<br/>期望 H = 0.8 × 0.88 + 0.2 × 0.29 = 0.76 bits。"
    },
    {
    ...
    },
    {
        ...
        "solution_en": "(a) Neighbouring times highly correlated (data leakage).<br/>(b) Use time-series split (train past, test future) or different days.",
        "solution_cn": "(a) 相邻时间高度相关（数据泄漏）。<br/>(b) 使用时间序列分割（训练过去，测试未来）或不同的日期。"
    },
    {
    ...
    },
    {
        ...
        "solution_cn": "H[state|vib] = 0.75.<br/>IG = 0.95 - 0.75 = 0.2 bits。"
    },
    {
    ...
    },
    {
        ...
        "solution_en": "1. Bootstrap samples.<br/>2. Build trees.<br/>3. Subspace sampling (random feature subset) at each split.<br/>4. Aggregate predictions.",
        "solution_cn": "1. Bootstrap 采样。<br/>2. 构建树。<br/>3. 每个分裂处的子空间采样（随机特征子集）。<br/>4. 聚合预测。"
    },
    {
        "topic": "Linear Regression SSE (LR Q1)",
        "question_en": "Write sum-squared error (SSE) in vector notation.",
        "question_cn": "用向量符号写出平方误差和 (SSE)。",
        "solution_en": "(y - Xw)<super>T</super>(y - Xw).",
        "solution_cn": "(y - Xw)<super>T</super>(y - Xw)。"
    },
    {
    ...
    },
    {
        ...
        "solution_en": "SSE grad: -2X<super>T</super>(y - Xw).<br/>MAE grad: -1/N ∑ [X]<sub>i:</sub> sgn(y<sub>i</sub> - [X]<sub>i:</sub>w).",
        "solution_cn": "SSE 梯度：-2X<super>T</super>(y - Xw)。<br/>MAE 梯度：-1/N ∑ [X]<sub>i:</sub> sgn(y<sub>i</sub> - [X]<sub>i:</sub>w)。"
    },
    {
        ...
        "solution_en": "Rows are [1, x, x<super>2</super>, x<super>3</super>].<br/>E.g., row 1: [1, 2, 4, 8].",
        "solution_cn": "行是 [1, x, x<super>2</super>, x<super>3</super>]。<br/>例如，第 1 行：[1, 2, 4, 8]。"
    },
    {
        ...
        "solution_en": "Add λw<super>T</super> w.",
        "solution_cn": "添加 λw<super>T</super> w。"
    },
    {
        "topic": "Gaussian Process Covariance (GP Q1)",
        "question_en": "Exponential kernel k(x,x')=e<super>-|x-x'|/10</super>. Train: (1,2), (3,4). Test: 2. Compute k<sub>*f</sub> and K<sub>ff</sub>.",
        "question_cn": "指数核 k(x,x')=e<super>-|x-x'|/10</super>。训练点：(1,2), (3,4)。测试点：2。计算 k<sub>*f</sub> 和 K<sub>ff</sub>。",
        "solution_en": "k<sub>*f</sub> ≈ [0.90, 0.90].<br/>K<sub>ff</sub> ≈ [[1, 0.82], [0.82, 1]].<br/>Posterior mean ≈ 2.99.",
        "solution_cn": "k<sub>*f</sub> ≈ [0.90, 0.90].<br/>K<sub>ff</sub> ≈ [[1, 0.82], [0.82, 1]].<br/>后验均值 ≈ 2.99。"
    },
    {
...
print("JSON file created: revision_questions.json with fixed unicode/tags.")
